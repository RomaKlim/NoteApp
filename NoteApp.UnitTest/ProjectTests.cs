...
namespace ProjectTests
{
    ...
    internal class ProjectManagerTest
    {
        ...
        public void TestSaveToFile_CorrectValue()
        {
            ...
            string fileName = path + "/" + example.NameList+".nt";
        ...
        public void TestLoadFromFile_CorrectValue()
        {
            ...
            string path = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            string fileName = @"/etalon.nt";
            var expected = ProjectManager.Deserializer(path, fileName);
            for (int i = 0; i < actual.ListNote.Count; i++)
            {
               ...
               "Сравнение результата десериализованного обьекта и ожидаемого");
                ...
                Assert.AreEqual(actual.ListNote[i].DateChange, expected.ListNote[i].DateChange,
               "Сравнение результата десериализованного обьекта и ожидаемого");

                Assert.AreEqual(actual.ListNote[i].DateCreate, expected.ListNote[i].DateCreate,
               "Сравнение результата десериализованного обьекта и ожидаемого");
            }
        }
    }
